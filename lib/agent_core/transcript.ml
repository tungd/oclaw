...
let init_db db =
  let exec sql = match Sqlite3.exec db sql with
    | Sqlite3.Rc.OK -> ()
    | rc -> Printf.eprintf "DB INIT ERROR: %s\n" (Sqlite3.Rc.to_string rc); failwith (Printf.sprintf "DB error [%s]" (Sqlite3.Rc.to_string rc))
  in
  
  
  exec "CREATE TABLE IF NOT EXISTS conversations (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, parent_chat_id INTEGER NULL, parent_node_id INTEGER NULL, timestamp REAL NOT NULL)";
  exec "CREATE TABLE IF NOT EXISTS transcripts (id INTEGER PRIMARY KEY AUTOINCREMENT, chat_id INTEGER NOT NULL, path TEXT NOT NULL, kind TEXT NOT NULL, content TEXT NOT NULL, model TEXT NULL, metadata TEXT NOT NULL, timestamp REAL NOT NULL, FOREIGN KEY(chat_id) REFERENCES conversations(id))";
  exec "CREATE INDEX IF NOT EXISTS idx_transcripts_chat ON transcripts(chat_id)";
